...
import {IChildProcessModule} from "child-process-promise";
import { Readable } from "stream";

describe("exec", () => {
  let exec: IExecModule;
  let cpp: Mocked<IChildProcessModule>;
...
describe("spawn", () => {
  let exec: IExecModule;
  let cpp: Mocked<IChildProcessModule>;
  let log: Mocked<npmlog.INPMLogModule>;
...
function toStream(string: string) {
...
function returnSuccessfulWithValue(value: any) {
  const result = {
...
function returnErrorWithValue(value: any) {
  const result = {
    stdout: "out",
    stderr: value,
    childProcess: {
...
function mockCppSuccessful(cpp: Mocked<IChildProcessModule>) {
  const successfulResult = returnSuccessfulWithValue("successful result");
  const resolvedPromise = Promise.resolve(successfulResult);
  cpp.exec.mockReturnValueOnce(resolvedPromise);
